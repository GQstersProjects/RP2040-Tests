...
int endpoint = 180; // Multiplier for servo endpoints. Should be 180 on ESC.
int speed = 8 * endpoint;
int trim = 0;
int neutral = 4915; // Servo Neutral Point. Should be 4915 on ESC. This value is used to calculate a PWM value for the servo, and SHOULD NOT EVER BE ZERO.
int heartbeat = 0;
...
static esp_err_t cmd_handler(httpd_req_t *req)
{
    ...
    heartbeat = millis(); //Save time of last received command for failsafe.
    ...
    else if(!strcmp(variable, "trim")) 
    {
      ...
      if (val==1) {
        ...
        if      (actstate == fwd) { ledcWrite(3,neutral+trim+speed*.5); ledcWrite(4,neutral+trim-speed); }
        else if (actstate == rev) { ledcWrite(3,neutral-trim-speed*.5); ledcWrite(4,neutral-trim+speed); }
        else                      { ledcWrite(3,neutral-trim-speed*.7); ledcWrite(4,neutral+trim-speed*.7); }          
      }
      else if (val==4) {
        Serial.println("TurnRight");
        if      (actstate == fwd) { ledcWrite(3,neutral+trim+speed); ledcWrite(4,neutral+trim-speed*.5); }
        else if (actstate == rev) { ledcWrite(3,neutral-trim-speed); ledcWrite(4,neutral-trim+speed*.5); }
        else                      { ledcWrite(3,neutral+trim+speed*.7); ledcWrite(4,neutral-trim+speed*.7); }        
      }
      else {
        Serial.println("Stop");
    ...
    <img id="banner" src="data:image/png;base64,">
     <figure>
      ...
      <section id="buttons">
        ...
        <tr><td align="center">Trim</td><td align="center" colspan="2"><input type="range" id="trim" min="-192" max="192" value="0" onchange="try{fetch(document.location.origin+'/control?var=trim&val='+this.value);}catch(e){}"></td></tr>
